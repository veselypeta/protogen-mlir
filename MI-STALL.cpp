/******************************
  Program "MI-STALL.m" compiled by "Caching Murphi Release 5.4.9.1"
...
class mu_1_cache_state: public mu__byte
{
 public:
  ...
  mu_1_cache_state (const char *name, int os): mu__byte(9, 14, 3, name, os) {};
  mu_1_cache_state (void): mu__byte(9, 14, 3) {};
  mu_1_cache_state (int val): mu__byte(9, 14, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
const char *mu_1_cache_state::values[] = {"cache_I","cache_I_load","cache_I_store","cache_M","cache_M_evict","cache_I_evict",NULL };
...
class mu_1_directory_state: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_directory_state::values[ int(val) - 15] );
    else return ( s << "Undefined" );
  };

  mu_1_directory_state (const char *name, int os): mu__byte(15, 16, 2, name, os) {};
  mu_1_directory_state (void): mu__byte(15, 16, 2) {};
  mu_1_directory_state (int val): mu__byte(15, 16, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_Address: public mu__byte
{
 public:
  ...
  friend ostream& operator<< (ostream& s, mu_1_Address& val)
    {
      if (val.defined())
	return ( s << mu_1_Address::values[ int(val) - 17 ] );
      else
	return ( s << "Undefined" );
    };

  mu_1_Address (const char *name, int os): mu__byte(17, 17, 1, name, os) {};
  mu_1_Address (void): mu__byte(17, 17, 1) {};
  mu_1_Address (int val): mu__byte(17, 17, 1, "Parameter or function result.", 0)
    { operator=(val); };
...
class mu_1_OBJSET_cache: public mu__byte
{
 public:
  ...
  friend ostream& operator<< (ostream& s, mu_1_OBJSET_cache& val)
    {
      if (val.defined())
	return ( s << mu_1_OBJSET_cache::values[ int(val) - 18 ] );
      else
	return ( s << "Undefined" );
    };

  mu_1_OBJSET_cache (const char *name, int os): mu__byte(18, 20, 2, name, os) {};
  mu_1_OBJSET_cache (void): mu__byte(18, 20, 2) {};
  mu_1_OBJSET_cache (int val): mu__byte(18, 20, 2, "Parameter or function result.", 0)
    { operator=(val); };
  const char * Name() { return values[ value() -18]; };
  virtual void print()
    {
      if (defined()) cout << name << ':' << values[ value() - 18] << '\n';
      else cout << name << ":Undefined\n";
    };
  void print_statistic() {};
...
class mu_1_OBJSET_directory: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_OBJSET_directory::values[ int(val) - 21] );
    else return ( s << "Undefined" );
  };

  mu_1_OBJSET_directory (const char *name, int os): mu__byte(21, 21, 1, name, os) {};
  mu_1_OBJSET_directory (void): mu__byte(21, 21, 1) {};
  mu_1_OBJSET_directory (int val): mu__byte(21, 21, 1, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_Machines: public mu__byte
{
 public:
  ...
  int indexvalue()
  {
    if ((value() >= 18) && (value() <= 20)) return (value() - 18);
    if ((value() >= 21) && (value() <= 21)) return (value() - 18);
  return 0;
  };
  inline int unionassign(int val)
  {
    if (val >= 0 && val <= 2) return value(val+18);
    if (val >= 3 && val <= 3) return value(val+18);
  return 0;
  };
 ...
 public:
  ...
  mu_1_ENTRY_cache& operator[] (int index) /* const */
  {
	...
	if (index==UNDEFVAL) 
	  ...
	  Error.Error("Funny index value %d for %s: Address is internally represented from 17 to 17.\nInternal Error in Type checking.",index, name);
	return array[0];
      }
#else
    return array[ index - 17 ];
 ...
 public:
  ...
  mu_1_MACH_cache& operator[] (int index) /* const */
  {
	...
	if (index==UNDEFVAL) 
	  ...
	  Error.Error("Funny index value %d for %s: OBJSET_cache is internally represented from 18 to 20.\nInternal Error in Type checking.",index, name);
	return array[0];
      }
#else
    return array[ index - 18 ];
...
const int mu_cache_I_evict = 14;
const int mu_directory_I = 15;
const int mu_directory_M = 16;
const int mu_Address_1 = 17;
const int mu_OBJSET_cache_1 = 18;
const int mu_OBJSET_cache_2 = 19;
const int mu_OBJSET_cache_3 = 20;
const int mu_directory = 21;
...
mu_msg = mu_Resp( mu_adr, mu_GetM_Ack_D, (int)mu_m, mu_inmsg.mu_src, mu_cache_entry.mu_cl );
mu_Send_resp ( mu_msg );
mu_cache_entry.mu_State = mu_cache_I_evict;
break;
default:
return mu_false;
break;
...
public:
  ...
  mu_1_ENTRY_cache& mu_cle = mu_i_cache[mu_m].mu_CL[mu_adr];
    return (mu_cle.mu_State) == (mu_cache_M);
  }
...
public:
  ...
  bool Condition(unsigned r)
  {
    ...
    r = r / 3;
  mu_1_ENTRY_cache& mu_cle = mu_i_cache[mu_m].mu_CL[mu_adr];
    return (mu_cle.mu_State) == (mu_cache_I);
  }
...
public:
  ...
  {
{
for(int mu_i = 18; mu_i <= 20; mu_i++) {
{
for(int mu_a = 17; mu_a <= 17; mu_a++) {
...
mu_fwd.undefine();
{
for(int mu_n = 18; mu_n <= 21; mu_n++)
  if (( ( mu_n >= 21 ) && ( mu_n <= 21 ) )|| ( ( mu_n >= 18 ) && ( mu_n <= 20 ) )) {
mu_cnt_fwd[mu_n] = 0;
};
};
mu_resp.undefine();
...
: Presentation(Simple)
{
 ...
 revperm_mu_1_OBJSET_cache = new arr_mu_1_OBJSET_cache[6];
    ...
    for (i=0; i<6; i++)
      for (j=18; j<=20; j++)
        for (k=18; k<=20; k++)
          if (revperm_mu_1_OBJSET_cache[i][k-18]==j)   // k - base 
            perm_mu_1_OBJSET_cache[i][j-18]=k; // j - base 
...
void PermSet::SimpleToExplicit()
{
  ...
  for (i=0; i<6; i++) // to be in or not to be
    for (k=0; k<3; k++) // step through class_mu_1_pid[k]
      if (! (perm_mu_1_OBJSET_cache[i][k]-18 >=start_mu_1_OBJSET_cache[k] 
	     && perm_mu_1_OBJSET_cache[i][k]-18 < start_mu_1_OBJSET_cache[k] + size_mu_1_OBJSET_cache[k]) )
  	    {
  ...
  for (j=0; j<=undefined_class_mu_1_OBJSET_cache; j++) // class number
    {
      for (k=0; k<3; k++) // step through class_mu_1_pid[k]
	    if (class_mu_1_OBJSET_cache[k]==j)
	      revperm_mu_1_OBJSET_cache[0][start++] = k+18;
    }
  for (j=0; j<3; j++)
    for (k=0; k<3; k++)
      if (revperm_mu_1_OBJSET_cache[0][k]==j+18)
        perm_mu_1_OBJSET_cache[0][j]=k+18;
  Presentation = Explicit;
}
...
void PermSet::GenPerm0(mu_1_OBJSET_cache* Perm,int size, unsigned long& count)
{
  ...
  if (size!=3)
    ...
    {
      for (i=18; i<=20; i++)
        revperm_mu_1_OBJSET_cache[count][i-18]=Perm[i-18].value();// i - base
      count++;
    }
}
...
bool PermSet::NextPermutation()
{
  ...
  for (start = 0; start < 3; )
    {
      end = start-1+size_mu_1_OBJSET_cache[revperm_mu_1_OBJSET_cache[0][start]-18];
...
if (!nexted) return FALSE;
  for (j=0; j<3; j++)
    for (k=0; k<3; k++)
      if (revperm_mu_1_OBJSET_cache[0][k]==j+18)   // k - base 
	perm_mu_1_OBJSET_cache[0][j]=k+18; // j - base 
  return TRUE;
}
...
void mu_1_OBJSET_cache::Permute(PermSet& Perm, int i)
{
  ...
  if (defined())
    value(Perm.perm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][value()-18]); // value - base
};
void mu_1_OBJSET_cache::SimpleCanonicalize(PermSet& Perm)
{
  ...
  if (defined())
    if (Perm.class_mu_1_OBJSET_cache[value()-18]==Perm.undefined_class_mu_1_OBJSET_cache) // value - base
      {
        // it has not been mapped to any particular value
        for (i=0; i<3; i++)
          if (Perm.class_mu_1_OBJSET_cache[i] == Perm.undefined_class_mu_1_OBJSET_cache && i!=value()-18)
            Perm.class_mu_1_OBJSET_cache[i]++;
        value(18 + Perm.undefined_class_mu_1_OBJSET_cache++);
      }
    else 
      {
        value(Perm.class_mu_1_OBJSET_cache[value()-18]+18);
      }
}
...
void mu_1_Machines::Permute(PermSet& Perm, int i)
{
  ...
  if (defined()) {
    if ( ( value() >= 18 ) && ( value() <= 20 ) )
      value(Perm.perm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][value()-18]+(0)); // value - base
  }
}
...
void mu_1_OBJ_cache::Permute(PermSet& Perm, int i)
{
  ...
  temp = *this;
  for (j=18; j<=20; j++)
    (*this)[j] = temp[Perm.revperm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][j-18]];};
...
void mu_1_OBJ_cache::Canonicalize(PermSet& Perm)
{
      ...
      for (i=0; i<3; i++)
        {
          for (j=0; j<count_mu_1_OBJSET_cache; j++)
            {
              compare = CompareWeight(value[j],(*this)[i+18]);
              if (compare==0)
                {
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache())
  ...
  else
    {
      ...
      temp = *this;
      for (j=0; j<3; j++)
        array[Perm.class_mu_1_OBJSET_cache[j]+0] = temp[j+18];
    }
}
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
    {
      ...
      for (k=0; k<3; k++) // step through class
        if ((*this)[k+18][i0].mu_src.isundefined()
            ||(*this)[k+18][i0].mu_src!=k+18)
          exists = TRUE;
        else
          split = TRUE;
      if (exists && split)
          ...
          for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
            {
              ...
              for (k=0; k<3; k++) // step through class
                if (pos_mu_1_OBJSET_cache[i][k] 
                    && !(*this)[k+18][i0].mu_src.isundefined()
                    && (*this)[k+18][i0].mu_src==k+18)
                  {
                    exists = TRUE;
                    goodset_mu_1_OBJSET_cache[k] = TRUE;
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
    {
      ...
      // if there exists both self loop and non-self loop
      for (k=0; k<3; k++) // step through class
        if ((*this)[k+18][i0].mu_dst.isundefined()
            ||(*this)[k+18][i0].mu_dst!=k+18)
          exists = TRUE;
        else
          split = TRUE;
      if (exists && split)
          ...
          for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
            {
              ...
              for (k=0; k<3; k++) // step through class
                if (pos_mu_1_OBJSET_cache[i][k] 
                    && !(*this)[k+18][i0].mu_dst.isundefined()
                    && (*this)[k+18][i0].mu_dst==k+18)
                  {
                    exists = TRUE;
                    goodset_mu_1_OBJSET_cache[k] = TRUE;
      ...
      if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
        {
          ...
          for (k=0; k<3; k++) // step through class
            if (!(*this)[k+18][i0].mu_src.isundefined()
                &&(*this)[k+18][i0].mu_src!=k+18
                &&(*this)[k+18][i0].mu_src>=18
                &&(*this)[k+18][i0].mu_src<=20)
              exists = TRUE;
          if (exists)
            {
              for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
                  ...
                  for (j=0; j<count_mu_1_OBJSET_cache; j++) // for value priority
                    {
                      ...
                      for (k=0; k<3; k++) // step through class
                        if (pos_mu_1_OBJSET_cache[i][k] 
                            ...
                            && (*this)[k+18][i0].mu_src>=18
                            && (*this)[k+18][i0].mu_src<=20
                            && pos_mu_1_OBJSET_cache[j][(*this)[k+18][i0].mu_src-18])
                          {
                            exists = TRUE;
                            goodset_mu_1_OBJSET_cache[k] = TRUE;
                          }
      ...
      if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
        {
          ...
          for (k=0; k<3; k++) // step through class
            if (!(*this)[k+18][i0].mu_dst.isundefined()
                &&(*this)[k+18][i0].mu_dst!=k+18
                &&(*this)[k+18][i0].mu_dst>=18
                &&(*this)[k+18][i0].mu_dst<=20)
              exists = TRUE;
          if (exists)
            {
              for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
                  ...
                  for (j=0; j<count_mu_1_OBJSET_cache; j++) // for value priority
                    {
                      ...
                      for (k=0; k<3; k++) // step through class
                        if (pos_mu_1_OBJSET_cache[i][k] 
                            ...
                            && (*this)[k+18][i0].mu_dst>=18
                            && (*this)[k+18][i0].mu_dst<=20
                            && pos_mu_1_OBJSET_cache[j][(*this)[k+18][i0].mu_dst-18])
                          {
                            exists = TRUE;
                            goodset_mu_1_OBJSET_cache[k] = TRUE;
                          }
...
void mu_1_OBJ_Orderedcnt::Permute(PermSet& Perm, int i)
{
  ...
  temp = *this;
  for (j=18; j<=20; j++)
      (*this)[j].value(temp[Perm.revperm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][j-18]].value());
};
...
void mu_1_OBJ_Orderedcnt::Canonicalize(PermSet& Perm)
{
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache())
      ...
      for (i=0; i<3; i++)
        for (j=0; j<3; j++)
         if (i >=start_mu_1_OBJSET_cache[j] 
             && i < start_mu_1_OBJSET_cache[j] + size_mu_1_OBJSET_cache[j])
           {
             array[i+0].value(temp[j+18].value());
             break;
           }
    }
  else
    {
      ...
      temp = *this;
      for (j=0; j<3; j++)
        array[Perm.class_mu_1_OBJSET_cache[j]+0].value(temp[j+18].value());
    }
}
                ...
                for (j=0; j<count_mu_1_OBJSET_cache; j++) //scan through element priority
                  {
                    ...
                    for (k=0; k<current_size; k++) // scan array index
                      ...
                      if (pos_multisetindex[i][k] 
                          ...
                          && (*this)[k].mu_src>=18
                          && (*this)[k].mu_src<=20
                          && pos_mu_1_OBJSET_cache[j][(*this)[k].mu_src-18])
                        {
                          exists = TRUE;
                          goodset_multisetindex[k] = TRUE;
                          goodset_mu_1_OBJSET_cache[(*this)[k].mu_src-18] = TRUE;
                        }
                    if (exists)
                      {
                ...
                for (j=0; j<count_mu_1_OBJSET_cache; j++) //scan through element priority
                  {
                    ...
                    for (k=0; k<current_size; k++) // scan array index
                      ...
                      if (pos_multisetindex[i][k] 
                          ...
                          && (*this)[k].mu_dst>=18
                          && (*this)[k].mu_dst<=20
                          && pos_mu_1_OBJSET_cache[j][(*this)[k].mu_dst-18])
                        {
                          exists = TRUE;
                          goodset_multisetindex[k] = TRUE;
                          goodset_mu_1_OBJSET_cache[(*this)[k].mu_dst-18] = TRUE;
                        }
                    if (exists)
                      {
                        // set split for the array index type

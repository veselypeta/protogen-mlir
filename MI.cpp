/******************************
  Program "../MI.m" compiled by "Caching Murphi Release 5.4.9.1"
...
const char *mu_1_MessageType::values[] = {"GetM","Fwd_GetM","GetM_Ack_D","PutM","Put_Ack",NULL };
...
class mu_1_cache_state: public mu__byte
{
 public:
  ...
  mu_1_cache_state (const char *name, int os): mu__byte(9, 12, 3, name, os) {};
  mu_1_cache_state (void): mu__byte(9, 12, 3) {};
  mu_1_cache_state (int val): mu__byte(9, 12, 3, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
const char *mu_1_cache_state::values[] = {"cache_M","cache_I_M_D","cache_I","cache_M_I_Ack",NULL };
...
class mu_1_directory_state: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_directory_state::values[ int(val) - 13] );
    else return ( s << "Undefined" );
  };

  mu_1_directory_state (const char *name, int os): mu__byte(13, 14, 2, name, os) {};
  mu_1_directory_state (void): mu__byte(13, 14, 2) {};
  mu_1_directory_state (int val): mu__byte(13, 14, 2, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_Address: public mu__byte
{
 public:
  ...
  friend ostream& operator<< (ostream& s, mu_1_Address& val)
    {
      if (val.defined())
	return ( s << mu_1_Address::values[ int(val) - 15 ] );
      else
	return ( s << "Undefined" );
    };

  mu_1_Address (const char *name, int os): mu__byte(15, 15, 1, name, os) {};
  mu_1_Address (void): mu__byte(15, 15, 1) {};
  mu_1_Address (int val): mu__byte(15, 15, 1, "Parameter or function result.", 0)
    { operator=(val); };
  const char * Name() { return values[ value() -15]; };
  virtual void print()
    {
      if (defined()) cout << name << ':' << values[ value() - 15] << '\n';
...
class mu_1_OBJSET_cache: public mu__byte
{
 public:
  ...
  friend ostream& operator<< (ostream& s, mu_1_OBJSET_cache& val)
    {
      if (val.defined())
	return ( s << mu_1_OBJSET_cache::values[ int(val) - 16 ] );
      else
	return ( s << "Undefined" );
    };

  mu_1_OBJSET_cache (const char *name, int os): mu__byte(16, 18, 2, name, os) {};
  mu_1_OBJSET_cache (void): mu__byte(16, 18, 2) {};
  mu_1_OBJSET_cache (int val): mu__byte(16, 18, 2, "Parameter or function result.", 0)
    { operator=(val); };
...
class mu_1_OBJSET_directory: public mu__byte
{
 public:
  ...
  {
    if (val.defined())
      return ( s << mu_1_OBJSET_directory::values[ int(val) - 19] );
    else return ( s << "Undefined" );
  };

  mu_1_OBJSET_directory (const char *name, int os): mu__byte(19, 19, 1, name, os) {};
  mu_1_OBJSET_directory (void): mu__byte(19, 19, 1) {};
  mu_1_OBJSET_directory (int val): mu__byte(19, 19, 1, "Parameter or function result.", 0)
  {
     operator=(val);
  };
...
class mu_1_Machines: public mu__byte
{
 public:
  ...
  int indexvalue()
  {
    if ((value() >= 16) && (value() <= 18)) return (value() - 16);
    if ((value() >= 19) && (value() <= 19)) return (value() - 16);
  return 0;
  };
  inline int unionassign(int val)
  {
    if (val >= 0 && val <= 2) return value(val+16);
    if (val >= 3 && val <= 3) return value(val+16);
  return 0;
  };
...
void mu_1_ENTRY_cache::set_self(const char *n, int os)
{
  ...
  if (name) mu_Perm.set_self_2(name, ".Perm", os + 8 ); else mu_Perm.set_self_2(NULL, NULL, 0);
...
void mu_1_ENTRY_directory::set_self(const char *n, int os)
{
  ...
  if (name) mu_Perm.set_self_2(name, ".Perm", os + 8 ); else mu_Perm.set_self_2(NULL, NULL, 0);
  if (name) mu_cl.set_self_2(name, ".cl", os + 16 ); else mu_cl.set_self_2(NULL, NULL, 0);
  if (name) mu_owner.set_self_2(name, ".owner", os + 24 ); else mu_owner.set_self_2(NULL, NULL, 0);
}
 ...
 public:
  ...
  mu_1_ENTRY_cache& operator[] (int index) /* const */
  {
	...
	if (index==UNDEFVAL) 
	  ...
	  Error.Error("Funny index value %d for %s: Address is internally represented from 15 to 15.\nInternal Error in Type checking.",index, name);
	return array[0];
      }
#else
    return array[ index - 15 ];
 ...
 public:
  ...
  mu_1_MACH_cache& operator[] (int index) /* const */
  {
	...
	if (index==UNDEFVAL) 
	  ...
	  Error.Error("Funny index value %d for %s: OBJSET_cache is internally represented from 16 to 18.\nInternal Error in Type checking.",index, name);
	return array[0];
      }
#else
    return array[ index - 16 ];
...
const int mu_GetM = 4;
const int mu_Fwd_GetM = 5;
const int mu_GetM_Ack_D = 6;
const int mu_PutM = 7;
const int mu_Put_Ack = 8;
const int mu_cache_M = 9;
const int mu_cache_I_M_D = 10;
const int mu_cache_I = 11;
const int mu_cache_M_I_Ack = 12;
const int mu_directory_I = 13;
const int mu_directory_M = 14;
const int mu_Address_1 = 15;
const int mu_OBJSET_cache_1 = 16;
const int mu_OBJSET_cache_2 = 17;
const int mu_OBJSET_cache_3 = 18;
const int mu_directory = 19;
...
/*** begin multisetcount 0 declaration ***/
  ...
  for (mu_i = 0; ; mu_i=mu_i+1)
	...
	{
	  if ( mu_true ) mu__intexpr5++;
	}
      if (mu_i == 12-1) break;
    }
  }
/*** end multisetcount 0 declaration ***/
if ( !((mu__intexpr5) < (mu_U_NET_MAX)) ) Error.Error("Assertion failed: Too many messages");
...
{
int mu__ub7 = (mu_cnt_fwd[mu_n]) - (1);
for (int mu_i = 0; mu_i <= mu__ub7; mu_i += 1) {
...
mu_cache_entry.mu_Perm = mu_none;
break;
default:
return mu_false;
break;
...
mu_cache_entry.mu_Perm = mu_store;
...
mu_directory_entry.mu_Perm = mu_none;
break;
case mu_PutM:
...
mu_Send_req ( mu_msg );
mu_cle.mu_State = mu_cache_I_M_D;
...
{
  mu_1_ENTRY_cache& mu_cle = mu_i_cache[mu_m].mu_CL[mu_adr];
mu_msg = mu_Request( mu_adr, mu_PutM, (int)mu_m, (int)mu_directory );
mu_Send_req ( mu_msg );
mu_cle.mu_State = mu_cache_M_I_Ack;
mu_cle.mu_Perm = mu_none;
}
};
/*** end procedure declaration ***/
...
public:
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_n.unionassign(r % 4);
    r = r / 4;
    while (what_rule < 52 && mu_midx.value()<12 )
      {
...
public:
  ...
  void NextRule(unsigned & what_rule)
  {
    ...
    mu_n.unionassign(r % 4);
    r = r / 4;
    while (what_rule < 100 && mu_midx.value()<12 )
      {
...
void SetNextEnabledRule(unsigned & what_rule)
{
  ...
  if (what_rule>=4 && what_rule<52)
    { R1.NextRule(what_rule);
      if (what_rule<52) return; }
  if (what_rule>=52 && what_rule<100)
...
bool Condition(unsigned r)
{
  ...
  if (r>=4 && r<=51) return R1.Condition(r-4);
  if (r>=52 && r<=99) return R2.Condition(r-52);
...
void Code(unsigned r)
{
  ...
  if (r>=4 && r<=51) { R1.Code(r-4); return; } 
  if (r>=52 && r<=99) { R2.Code(r-52); return; } 
...
int Priority(unsigned short r)
{
  ...
  if (r>=4 && r<=51) { return R1.Priority(); } 
  if (r>=52 && r<=99) { return R2.Priority(); } 
...
char * Name(unsigned r)
{
  ...
  if (r>=4 && r<=51) return R1.Name(r-4);
  if (r>=52 && r<=99) return R2.Name(r-52);
...
public:
  ...
  {
{
for(int mu_i = 19; mu_i <= 19; mu_i++) {
{
for(int mu_a = 15; mu_a <= 15; mu_a++) {
...
mu_fwd.undefine();
{
for(int mu_n = 16; mu_n <= 19; mu_n++)
  if (( ( mu_n >= 19 ) && ( mu_n <= 19 ) )|| ( ( mu_n >= 16 ) && ( mu_n <= 18 ) )) {
mu_cnt_fwd[mu_n] = 0;
};
};
  };
...
/********************
  Invariant records
 ********************/
int mu__invariant_18() // Invariant "Write Serialization"
{
bool mu__quant19; 
mu__quant19 = TRUE;
{
for(int mu_c1 = 16; mu_c1 <= 18; mu_c1++) {
bool mu__quant20; 
mu__quant20 = TRUE;
{
for(int mu_c2 = 16; mu_c2 <= 18; mu_c2++) {
bool mu__quant21; 
mu__quant21 = TRUE;
{
for(int mu_a = 15; mu_a <= 15; mu_a++) {
bool mu__boolexpr22;
bool mu__boolexpr23;
  if (!((mu_c1) != (mu_c2))) mu__boolexpr23 = FALSE ;
  else {
  mu__boolexpr23 = ((mu_i_cache[mu_c1].mu_CL[mu_a].mu_Perm) == (mu_store)) ; 
}
  if (!(mu__boolexpr23)) mu__boolexpr22 = TRUE ;
  else {
  mu__boolexpr22 = ((mu_i_cache[mu_c2].mu_CL[mu_a].mu_Perm) != (mu_store)) ; 
}
if ( !(mu__boolexpr22) )
...
bool mu__condition_24() // Condition for Rule "Write Serialization"
{
...
{"Write Serialization", &mu__condition_24, NULL, },
};
...
: Presentation(Simple)
{
 ...
 revperm_mu_1_OBJSET_cache = new arr_mu_1_OBJSET_cache[6];
    ...
    for (i=0; i<6; i++)
      for (j=16; j<=18; j++)
        for (k=16; k<=18; k++)
          if (revperm_mu_1_OBJSET_cache[i][k-16]==j)   // k - base 
            perm_mu_1_OBJSET_cache[i][j-16]=k; // j - base 
...
void PermSet::SimpleToExplicit()
{
  ...
  for (i=0; i<6; i++) // to be in or not to be
    for (k=0; k<3; k++) // step through class_mu_1_pid[k]
      if (! (perm_mu_1_OBJSET_cache[i][k]-16 >=start_mu_1_OBJSET_cache[k] 
	     && perm_mu_1_OBJSET_cache[i][k]-16 < start_mu_1_OBJSET_cache[k] + size_mu_1_OBJSET_cache[k]) )
  	    {
  ...
  for (j=0; j<=undefined_class_mu_1_OBJSET_cache; j++) // class number
    {
      for (k=0; k<3; k++) // step through class_mu_1_pid[k]
	    if (class_mu_1_OBJSET_cache[k]==j)
	      revperm_mu_1_OBJSET_cache[0][start++] = k+16;
    }
  for (j=0; j<3; j++)
    for (k=0; k<3; k++)
      if (revperm_mu_1_OBJSET_cache[0][k]==j+16)
        perm_mu_1_OBJSET_cache[0][j]=k+16;
  Presentation = Explicit;
}
...
void PermSet::GenPerm0(mu_1_OBJSET_cache* Perm,int size, unsigned long& count)
{
  ...
  if (size!=3)
    ...
    {
      for (i=16; i<=18; i++)
        revperm_mu_1_OBJSET_cache[count][i-16]=Perm[i-16].value();// i - base
      count++;
    }
}
...
bool PermSet::NextPermutation()
{
  ...
  for (start = 0; start < 3; )
    {
      end = start-1+size_mu_1_OBJSET_cache[revperm_mu_1_OBJSET_cache[0][start]-16];
...
if (!nexted) return FALSE;
  for (j=0; j<3; j++)
    for (k=0; k<3; k++)
      if (revperm_mu_1_OBJSET_cache[0][k]==j+16)   // k - base 
	perm_mu_1_OBJSET_cache[0][j]=k+16; // j - base 
  return TRUE;
}
...
void mu_1_OBJSET_cache::Permute(PermSet& Perm, int i)
{
  ...
  if (defined())
    value(Perm.perm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][value()-16]); // value - base
};
void mu_1_OBJSET_cache::SimpleCanonicalize(PermSet& Perm)
{
  ...
  if (defined())
    if (Perm.class_mu_1_OBJSET_cache[value()-16]==Perm.undefined_class_mu_1_OBJSET_cache) // value - base
      {
        // it has not been mapped to any particular value
        for (i=0; i<3; i++)
          if (Perm.class_mu_1_OBJSET_cache[i] == Perm.undefined_class_mu_1_OBJSET_cache && i!=value()-16)
            Perm.class_mu_1_OBJSET_cache[i]++;
        value(16 + Perm.undefined_class_mu_1_OBJSET_cache++);
      }
    else 
      {
        value(Perm.class_mu_1_OBJSET_cache[value()-16]+16);
      }
}
...
void mu_1_Machines::Permute(PermSet& Perm, int i)
{
  ...
  if (defined()) {
    if ( ( value() >= 16 ) && ( value() <= 18 ) )
      value(Perm.perm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][value()-16]+(0)); // value - base
  }
}
...
void mu_1_OBJ_cache::Permute(PermSet& Perm, int i)
{
  ...
  temp = *this;
  for (j=16; j<=18; j++)
    (*this)[j] = temp[Perm.revperm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][j-16]];};
...
void mu_1_OBJ_cache::Canonicalize(PermSet& Perm)
{
      ...
      for (i=0; i<3; i++)
        {
          for (j=0; j<count_mu_1_OBJSET_cache; j++)
            {
              compare = CompareWeight(value[j],(*this)[i+16]);
              if (compare==0)
                {
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache())
  ...
  else
    {
      ...
      temp = *this;
      for (j=0; j<3; j++)
        array[Perm.class_mu_1_OBJSET_cache[j]+0] = temp[j+16];
    }
}
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
    {
      ...
      for (k=0; k<3; k++) // step through class
        if ((*this)[k+16][i0].mu_src.isundefined()
            ||(*this)[k+16][i0].mu_src!=k+16)
          exists = TRUE;
        else
          split = TRUE;
      if (exists && split)
          ...
          for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
            {
              ...
              for (k=0; k<3; k++) // step through class
                if (pos_mu_1_OBJSET_cache[i][k] 
                    && !(*this)[k+16][i0].mu_src.isundefined()
                    && (*this)[k+16][i0].mu_src==k+16)
                  {
                    exists = TRUE;
                    goodset_mu_1_OBJSET_cache[k] = TRUE;
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
    {
      ...
      // if there exists both self loop and non-self loop
      for (k=0; k<3; k++) // step through class
        if ((*this)[k+16][i0].mu_dst.isundefined()
            ||(*this)[k+16][i0].mu_dst!=k+16)
          exists = TRUE;
        else
          split = TRUE;
      if (exists && split)
          ...
          for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
            {
              ...
              for (k=0; k<3; k++) // step through class
                if (pos_mu_1_OBJSET_cache[i][k] 
                    && !(*this)[k+16][i0].mu_dst.isundefined()
                    && (*this)[k+16][i0].mu_dst==k+16)
                  {
                    exists = TRUE;
                    goodset_mu_1_OBJSET_cache[k] = TRUE;
      ...
      if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
        {
          ...
          for (k=0; k<3; k++) // step through class
            if (!(*this)[k+16][i0].mu_src.isundefined()
                &&(*this)[k+16][i0].mu_src!=k+16
                &&(*this)[k+16][i0].mu_src>=16
                &&(*this)[k+16][i0].mu_src<=18)
              exists = TRUE;
          if (exists)
            {
              for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
                  ...
                  for (j=0; j<count_mu_1_OBJSET_cache; j++) // for value priority
                    {
                      ...
                      for (k=0; k<3; k++) // step through class
                        if (pos_mu_1_OBJSET_cache[i][k] 
                            ...
                            && (*this)[k+16][i0].mu_src>=16
                            && (*this)[k+16][i0].mu_src<=18
                            && pos_mu_1_OBJSET_cache[j][(*this)[k+16][i0].mu_src-16])
                          {
                            exists = TRUE;
                            goodset_mu_1_OBJSET_cache[k] = TRUE;
                          }
      ...
      if (Perm.MTO_class_mu_1_OBJSET_cache() && count_mu_1_OBJSET_cache<3)
        {
          ...
          for (k=0; k<3; k++) // step through class
            if (!(*this)[k+16][i0].mu_dst.isundefined()
                &&(*this)[k+16][i0].mu_dst!=k+16
                &&(*this)[k+16][i0].mu_dst>=16
                &&(*this)[k+16][i0].mu_dst<=18)
              exists = TRUE;
          if (exists)
            {
              for (i=0; i<count_mu_1_OBJSET_cache; i++) // for value priority
                  ...
                  for (j=0; j<count_mu_1_OBJSET_cache; j++) // for value priority
                    {
                      ...
                      for (k=0; k<3; k++) // step through class
                        if (pos_mu_1_OBJSET_cache[i][k] 
                            ...
                            && (*this)[k+16][i0].mu_dst>=16
                            && (*this)[k+16][i0].mu_dst<=18
                            && pos_mu_1_OBJSET_cache[j][(*this)[k+16][i0].mu_dst-16])
                          {
                            exists = TRUE;
                            goodset_mu_1_OBJSET_cache[k] = TRUE;
                          }
...
void mu_1_OBJ_Orderedcnt::Permute(PermSet& Perm, int i)
{
  ...
  temp = *this;
  for (j=16; j<=18; j++)
      (*this)[j].value(temp[Perm.revperm_mu_1_OBJSET_cache[Perm.in_mu_1_OBJSET_cache[i]][j-16]].value());
};
...
void mu_1_OBJ_Orderedcnt::Canonicalize(PermSet& Perm)
{
  ...
  if (Perm.MTO_class_mu_1_OBJSET_cache())
      ...
      for (i=0; i<3; i++)
        for (j=0; j<3; j++)
         if (i >=start_mu_1_OBJSET_cache[j] 
             && i < start_mu_1_OBJSET_cache[j] + size_mu_1_OBJSET_cache[j])
           {
             array[i+0].value(temp[j+16].value());
             break;
           }
    }
  else
    {
      ...
      temp = *this;
      for (j=0; j<3; j++)
        array[Perm.class_mu_1_OBJSET_cache[j]+0].value(temp[j+16].value());
    }
}
                ...
                for (j=0; j<count_mu_1_OBJSET_cache; j++) //scan through element priority
                  {
                    ...
                    for (k=0; k<current_size; k++) // scan array index
                      ...
                      if (pos_multisetindex[i][k] 
                          ...
                          && (*this)[k].mu_src>=16
                          && (*this)[k].mu_src<=18
                          && pos_mu_1_OBJSET_cache[j][(*this)[k].mu_src-16])
                        {
                          exists = TRUE;
                          goodset_multisetindex[k] = TRUE;
                          goodset_mu_1_OBJSET_cache[(*this)[k].mu_src-16] = TRUE;
                        }
                    if (exists)
                      {
                ...
                for (j=0; j<count_mu_1_OBJSET_cache; j++) //scan through element priority
                  {
                    ...
                    for (k=0; k<current_size; k++) // scan array index
                      ...
                      if (pos_multisetindex[i][k] 
                          ...
                          && (*this)[k].mu_dst>=16
                          && (*this)[k].mu_dst<=18
                          && pos_mu_1_OBJSET_cache[j][(*this)[k].mu_dst-16])
                        {
                          exists = TRUE;
                          goodset_multisetindex[k] = TRUE;
                          goodset_mu_1_OBJSET_cache[(*this)[k].mu_dst-16] = TRUE;
                        }
                    if (exists)
                      {
                        // set split for the array index type
